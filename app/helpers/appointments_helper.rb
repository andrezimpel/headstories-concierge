      ...
      events.each do |e|
        ical.event do |event|
          if e.start.present? && e.start
            event.dtstart = e.start.to_time.in_time_zone('Berlin').strftime("%Y%m%dT%H%M%S")
          elsif e.start.present?
            event.dtstart = e.start.to_time.in_time_zone('Berlin').strftime("%Y%m%d")
          end
          if e.end.present? && e.end
            event.dtend = e.end.to_time.in_time_zone('Berlin').strftime("%Y%m%dT%H%M%S")
          elsif e.end.present?
            # in ICS müssen ganztägige Termine bis 0 Uhr des nächsten Tages gehen
            event.dtend = (e.end.to_time.in_time_zone('Berlin') + 1.day).strftime("%Y%m%d")
          end
